...
NAMESPACE=`$KUBECTL get statefulset --all-namespaces | grep pure-provisioner | awk '{print $1}' -`
if [ -z $NAMESPACE ]; then
...
$KUBECTL rollout status sts  pure-provisioner -n $NAMESPACE >/dev/null 2>&1 
